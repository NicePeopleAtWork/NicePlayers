 ...
 *
 *  - pinger: (YBTimer *) The <YBTimer> from where the callback is being invoked.
 *  - diffTime: (double) the time difference between the previous call.
 */
typedef void (^PingerCallback) (YBTimer * pinger, double diffTime);
 ...
 */
- (instancetype)initWithCallback:(PingerCallback) callback;
/**
 * Init
 * @param callback the block to execute every <interval> milliseconds
 ...
 * @returns an instance of YBTimer
 */
- (instancetype)initWithCallback:(PingerCallback) callback andInterval:(double) interval;
