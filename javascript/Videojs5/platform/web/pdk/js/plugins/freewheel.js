...
$pdk.ns("$pdk.plugin.freewheel.admanager");
$pdk.plugin.freewheel.admanager.FlashAdManager=$pdk.extend($pdk.plugin.freewheel.admanager.NativeAdManager,{constructor:function(d,a,f,c,b){$pdk.plugin.freewheel.admanager.FlashAdManager.superclass.constructor(d,a,f);
this.ready=false;
this._callback=f;
var e=this;
this.addEventListener("urn:theplatform:freewheel:ready",function(g){if(g){e.ready=true;
e._callback.call()
}});
if(c){a.loadLayer("flash","freewheel",c+(c.indexOf("?")>0?"&":"?")+"adManagerUrl="+escape(d))
}else{a.loadLayer("flash","freewheel",$pdk.scriptRoot+"/swf/FreeWheelAdManagerLoader.swf?adManagerUrl="+escape(d))
}$pdk.ns("tv.freewheel");
...
if(!this._paused&&this._pauseSetCallback&&this._active){this._paused=true;
tpDebug(a+"- calling pauseSetCallback with True");
...
},next:function(a){},end:function(){this.clip=null;
...
this.clip.baseClip.contentCustomData["fw:titleId"]=a.titleId;
this.clip.baseClip.contentCustomData["fw:advertiserId"]=a.advertiserId;
this.clip.baseClip.contentCustomData["fw:siteSectionId"]=a.siteSectionId;
this.clip.baseClip.contentCustomData["fw:siteSectionTag"]=a.siteSectionTag;
this.clip.baseClip.contentCustomData["fw:adurl"]=a.adURL;
this.clip.baseClip.contentCustomData["fw:time_position_class"]=a.time_position_class;
this.clip.baseClip.contentCustomData["fw:base_unit"]=a.base_unit;
this.clip.baseClip.contentCustomData["fw:content_type"]=a.content_type;
...
}}},onMediaStart:function(b){this.log("onMediaStart");
if(this.adManager&&b.data&&b.data.baseClip.isAd===false&&b.data.chapter&&b.data.chapter.index===0){this.adManager.setVideoState(tv.freewheel.SDK.VIDEO_STATE_PLAYING);
if(b.data.baseClip.bitrate>0){this.setDesiredBitrate(b.data.baseClip.bitrate)
}if(this._pauseListener){this.proxy.removeEventListener("pause",this._pauseListener);
this._pauseListener=null
}if(this._unpauseListener){this.proxy.removeEventListener("play",this._unpauseListener);
this._unpauseListener=null
}}else{if(this.adManager instanceof $pdk.plugin.freewheel.admanager.JavaScriptAdManager&&$pdk.isIOS){this.proxy=this.controller.getVideoProxy();
var a=this;
this._pauseListener=function(){a.controller.dispatchEvent("OnPlayerPause",{globalDataType:"com.theplatform.pdk.data::MediaPause",clip:b.data,userInitiated:true})
};
this._unpauseListener=function(){a.controller.dispatchEvent("OnPlayerUnPause",{globalDataType:"com.theplatform.pdk.data::MediaPause",clip:b.data,userInitiated:true})
};
this.proxy.addEventListener("pause",this._pauseListener);
this.proxy.addEventListener("play",this._unpauseListener)
...
}},onSlotEnded:function(b){if(this.isTemporalSlot(b.slot)){this.log("Slot ended",tpConsts.INFO);
