...
?>
	...
	<body>
		...
		<?php if ($_GET['plugin'] != ""): ?>
			<div style="background-color: aliceblue; padding: 20px">
				<?php
					...
					$dir = '../../adnalyzers/' . strtolower($_GET['plugin']);
					if(!file_exists($dir. '/src/'))
						mkdir($dir . '/src/', 0777, true);

					// Copy files
					copyFile('adnalyzer.js', $dir . '/src/adnalyzer.js');
				...
				<p>Scaffold created at '
					<?php echo sprintf("adnalyzers/%s/", strtolower($_GET['plugin'])); ?>'.</p>
		...
		<table>
			...
			<tr>
				<td>Plugin:</td>
				<td><input type="text" name="plugin" placeholder="Freewheel" /></td>
			</tr>
			<tr>
				<td>Author:</td>
