 ...
 */
$YB.AdsApi = function(plugin) { // constructor
    /** Reference to the {@link $YB.Api|plugin} from where it was called. */
    this.plugin = plugin;
...
$YB.AdsApi.prototype.sendAdStart = function(params, callback) {
    try {
        if (this.plugin.isStartSent) {
            ...
            this.plugin.isAdStartSent = true;
            ...
            params.campaign = $YB.utils.getFirstDefined(params.campaign, this.plugin.data.ads.campaign);
            params.title = $YB.utils.getFirstDefined(params.title, this.getAdTitle());
            params.adDuration = $YB.utils.getFirstDefined(params.adDuration, this.plugin.data.ads.duration);
            params.playhead = $YB.utils.getFirstDefined(params.playhead, this.plugin.getAdPlayhead());

            if (typeof params.durationJointime != 'undefined') {
                this.plugin.isAdJoinSent = true;
            }

            this.position = params.position;

            this.plugin.comm.sendRequest('/adStart', params, callback);
...
$YB.AdsApi.prototype.sendAdJoin = function(params, callback) {
    try {
        if (this.plugin.isAdStartSent && !this.plugin.isAdJoinSent) {
            this.plugin.isAdJoinSent = true;
            ...
            this.plugin.comm.sendRequest('/adJoinTime', params, callback);
...
$YB.AdsApi.prototype.sendAdStop = function(params, callback) {
    try {
        if (this.plugin.isAdStartSent) {
            this.plugin.isAdStartSent = false;
            this.plugin.isAdJoinSent = false;
            this.plugin.isAdBuffering = false;
            ...
            if (this.position == 'pre' && !this.plugin.isJoinSent) {
                this.totalPrerollTime += params.totalDuration;
            }

            this.plugin.comm.sendRequest('/adStop', params, callback);
...
$YB.AdsApi.prototype.sendAdBufferStart = function() {
    try {
        if (this.plugin.isAdJoinSent && !this.plugin.isAdBuffering) {
            this.plugin.isAdBuffering = true;
...
$YB.AdsApi.prototype.sendAdBufferEnd = function(params, callback) {
    try {
        if (this.plugin.isAdJoinSent && this.plugin.isAdBuffering) {
            ...
            this.plugin.commsendRequest('/adBufferUnderrun', params, callback);
...
$YB.AdsApi.prototype.getAdResource = function() {
    try {
        ...
        } else if (typeof this.plugin.ads != 'undefined' && typeof this.plugin.ads.getAdResource == 'function') {
            return this.plugin.ads.getAdResource();
        } else {
            return '';
        }
    } catch (err) {
...
$YB.AdsApi.prototype.getAdPlayhead = function() {
    try {
        if (typeof this.plugin.ads != 'undefined' && typeof this.plugin.ads.getAdPlayhead == 'function') {
            return this.plugin.ads.getAdPlayhead();
        } else {
            return 0;
        }
    } catch (err) {
...
$YB.AdsApi.prototype.getAdPosition = function() {
    try {
        ...
        } else if (typeof this.plugin.ads != 'undefined' && typeof this.plugin.ads.getAdPosition == 'function') {
            return this.plugin.ads.getAdPosition();
        } else {
            return 'unknown';
        }
    } catch (err) {
...
$YB.AdsApi.prototype.getAdTitle = function() {
    try {
        ...
        } else if (typeof this.plugin.ads != 'undefined' && typeof this.plugin.ads.getAdTitle == 'function') {
            return this.plugin.ads.getAdTitle();
        } else {
            return '';
        }
    } catch (err) {
...
$YB.AdsApi.prototype.getAdDuration = function() {
    try {
        if (typeof this.plugin.data.ads.duration != "undefined") {
            return this.plugin.data.ads.duration;
        } else if (typeof this.plugin.ads != 'undefined' && typeof this.plugin.ads.getAdDuration == 'function') {
            var d = this.plugin.ads.getAdDuration();
...
$YB.AdsApi.prototype.getAdBitrate = function() {
    try {
        if (typeof this.plugin.ads != 'undefined' && typeof this.plugin.ads.getAdBitrate == 'function') {
            return Math.round(this.plugin.ads.getAdBitrate());
        } else {
            return -1;
        }
    } catch (err) {
...
$YB.PingApi.prototype.sendPing = function(params, callback) {
    try {
        ...
        // Ads
        if (this.plugin.isStartSent) {
            params.adPlayhead = $YB.utils.getFirstDefined(params.adPlayhead, this.plugin.adsApi.getAdPlayhead());
            params.adBitrate = $YB.utils.getFirstDefined(params.adBitrate, this.plugin.adsApi.getAdBitrate());
        }
...
$YB.VideoApi.prototype.sendStart = function(params, callback) {
    try {
        ...
        params.title = $YB.utils.getFirstDefined(params.title, this.plugin.data.media.title);
        ...
        params.cdn = $YB.utils.getFirstDefined(params.cdn, this.plugin.data.network.cdn);
        ...
        if (this.plugin.data.resumeConfig.enabled) { // If Resume service is enabled
...
$YB.VideoApi.prototype._consolidateTitle = function() {
    try {
        if (this.plugin.data && this.plugin.data.media && this.plugin.data.media.title) {
            if (this.plugin.data.properties.content_metadata) {
                this.plugin.data.properties.content_metadata.title = this.plugin.data.media.title;
            } else {
                this.plugin.data.properties.content_metadata = {
                    title: this.plugin.data.media.title
                };
            }
        }
    } catch (err) {
...
$YB.VideoApi.prototype.sendJoin = function(params, callback) {
    try {
        if (this.plugin.isStartSent && !this.plugin.isJoinSent) {
            ...
            if (this.plugin.adsApi.totalPrerollTime > 0) {
                params.time = params.time - this.plugin.adsApi.totalPrerollTime;
            }
...
$YB.VideoApi.prototype.getMediaDuration = function() {
    try {
        ...
        return Math.round($YB.utils.parseNumber(res));
    } catch (err) {
        $YB.warn(err);
        return 0;
    }
...
$YB.VideoApi.prototype.getPlayerVersion = function() {
    try {
        var res = "";
        if (typeof this.plugin.data.playerVersion != "undefined") {
            res = this.plugin.data.playerVersion;
        } else if (typeof this.plugin.getPlayerVersion == "function" && this.plugin.getPlayerVersion()) {
...
/**
 * Starts Nicebuffer in video and ads APIs.
 */
 ...
 */
$YB.services.Concurrency = function(plugin, interval) {
    try {
        this.plugin = plugin;
        ...
        this.data = this.plugin.data; // Save reference
        this.config = this.plugin.data.concurrencyConfig; // Save reference
...
$YB.services.Concurrency.prototype._checkConcurrency = function() {
    try {
        ...
        var context = this;
        this.plugin.comm.sendService(this.config.service, options, function(httpEvent) {
            if (httpEvent.response === "1") {
                // Concurrency collision, kick user.
                context.plugin.handleError({
                    errorCode: 14000,
                    msg: "CC_KICK"
                });
 ...
 * @param {$YB.plugins.Generic} plugin The plugin from where it was called.
 */
$YB.services.Resume = function(plugin) {
    try {
        this.plugin = plugin;
        this.timer = null;
        this.isResumed = 0;
        ...
        this.data = this.plugin.data; // Save reference
        this.config = this.plugin.data.resumeConfig; // Save reference
...
$YB.services.Resume.prototype._check = function() {
    try {
        if (this.config.enabled && typeof this.config.contentId != "undefined" && typeof this.data.username != "undefined") {
            ...
            var context = this;
            this.plugin.comm.sendService(this.config.service, {
...
$YB.services.Resume.prototype._sendPlayTime = function() {
    try {
        if (this.config.enabled && typeof this.config.contentId != "undefined" && typeof this.data.username != "undefined") {
            this.plugin.comm.sendService(this.config.playTimeService, {
                contentId: this.config.contentId,
                userId: this.data.username,
                playTime: this.plugin.getPlayhead()
            });
        }
    } catch (err) {
        $YB.error(err);
 ...
 * @param {$YB.plugins.Generic} plugin The plugin from where it was called.
 */
$YB.services.Smartswitch = function(plugin) {
    try {
        this.plugin = plugin;
        this.callback = function() {};

        this.data = this.plugin.data; // Save reference
        this.config = this.plugin.data.smartswitchConfig; // Save reference
...
$YB.utils.Buffer.prototype._checkBuffer = function() {
    try {
        if (this.api.plugin.isJoinSent && !this.api.plugin.isPaused && !this.api.plugin.isSeeking) {
 ...
 * This events will be listened: 'canplay', 'buffering', 'playing', 'waiting', 'ended', 'play', 'pause', 'error', 'abort',
 * 'seeking', 'seeked', 'stalled', 'suspend', 'dispose', 'loadeddata', 'loadstart', 'adstart' and 'adend'.
 * Events will be reported as level 4 messages.
 *
 * @memberof Debug-Util
 * @param o Object to attach the events.
 * @param [extraEvents] An array of extra events to watch. ie:  ['timeupdate', 'progress']
 * @param [id] ID of the event report.
 */
$YB.utils.listenAllEvents = function(o, extraEvents, id) {
    try {
        if ($YB.errorLevel >= 4) {
            ...
            var playerEvents = [
                'canplay', 'buffering', 'waiting', 'ended', 'play', 'playing', 'pause', 'resume', 'error', 'abort', 'seek', 'seeking', 'seeked',
                'stalled', 'dispose', 'loadeddata', 'loadstart', 'adstart', 'adend'
            ];
            if (extraEvents) {
            ...
            for (i = 0; i < playerEvents.length; i++) {
                if (typeof o == "function") {
                    o.call(window, playerEvents[i], debugEvent);
                } else if (o.on) {
                    o.on(playerEvents[i], debugEvent);
                } else if (o.addEventListener) {
                    o.addEventListener(playerEvents[i], debugEvent);
                }
            }
        }
    } catch (err) {
