...
try {
    ...
    $YB.util.listenAllEvents = function() {};
    $YB.util.serialize = function() {};
...
$YB.AjaxRequest = function(host, service, params, options) {
    try {
        ...
        if (typeof this.params == 'object') {
            this.params = this._parseParams(this.params);
        }
...
$YB.AjaxRequest.prototype.on = function(event, callback) {
    try {
        ...
        if (typeof callback == "function") {
            ...
            $YB.warn("Warning: Request '" + that.getUrl() + "' has a callback that is not a function.");
        }
    } catch (err) {
        $YB.error(err);
    } finally {
    ...
    try {
        ...
        this.parseTimeout = setTimeout(function() {
            if ('ResourceParser' in that.context.comm._preloaders) {
                ...
                $YB.warn('ResourceParser has exceded the maximum execution time (3s) and it will be aborted.');
            }
        }, 3000);
...
$YB.ResourceParser.prototype.getRealResource = function(resource) {
    try {
        ...
        if (extension.indexOf('.m3u8') === 0) {
            ...
            var that = this;
            new $YB.AjaxRequest(resource).load(function() {
                var response = this.responseText; //In this context, this referes to the XHR.
                var matches = /(.*(\.m3u8|\.ts|\.mp4)\?+.+)/i.exec(response); //get first line ending in .m3u8, .mp4 or .ts
...
$YB.Api.prototype.getThroughput = function() {
    try {
        if (typeof this.data.media.throughput != "undefined") {
            return this.data.media.throughput;
            ...
            return Math.round(this.plugin.getThroughput());
        } else {
            return -1;
        }
    } catch (err) {
