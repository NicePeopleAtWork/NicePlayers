...
/**
 * Tries to get the throughput of the ad with adnalyzer.getAdThroughput().
 * @return {number} Throughput or -1.
 */
$YB.api.Info.prototype.getAdThroughput = function() {
    try {
        return $YB.utils.parseNumber(Math.round(this.plugin.adnalyzer.getAdThroughput()), -1);
    } catch (err) {
        $YB.warn(err);
        return -1;
    }
...
/**
 * Override this function to return ad throughput.
 * @return {number} -1
 */
$YB.adnalyzers.Generic.prototype.getAdThroughput = function() {
    return -1;
};
...
$YB.utils.Buffer.prototype._checkBuffer = function() {
    try {
        if (this.api.plugin.isJoinSent && !this.api.plugin.isPaused && !this.api.plugin.isSeeking && !this.api.plugin.isShowingAds) {
            ...
            this._checkPlayhead(this.api.plugin.infoApi.getPlayhead(), function() {
                api.sendBufferStart()
            }, function() {
                api.sendBufferEnd();
            });
...
$YB.utils.Buffer.prototype._checkAdBuffer = function() {
    try {
        if (this.api.adnalyzer.isAdJoinSent && !this.api.adnalyzer.isAdPaused) {
            ...
            this._checkPlayhead(this.api.adnalyzer.plugin.infoApi.getAdPlayhead(), function() {
 ...
 */
$YB.utils.Buffer.prototype._checkPlayhead = function(currentPlayhead, cbStart, cbEnd) {
    try {
        ...
        } else if (this.lastPlayhead && !this.api.plugin.isBuffering && diffPlayhead < this.options.threshold) { // Video is buffering
            cbStart();
        }
    } catch (err) {
        $YB.error(err);
