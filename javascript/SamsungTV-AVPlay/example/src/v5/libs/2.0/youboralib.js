...
try {
    ...
    // Load debug mode from window.location.search, search for 'file.html?youbora-debug=4' to set the errorLevel and '?youbora-console=plain' to set plainConsole.
    ...
    (function() {
        if (typeof window != 'undefined') {
            ...
            m = /\?.*\&*(youbora-console=plain)/i.exec(window.location.search);
            if (m !== null) {
...
$YB.AjaxRequest.prototype.on = function(event, callback) {
    try {
        ...
        if (typeof callback == "function") {
            this.xmlHttp.addEventListener(event, callback, false);
        ...
        if (this.options.requestHeaders) {
            for (var key in this.options.requestHeaders) {
                if (this.options.requestHeaders.hasOwnProperty(key)) {
                    this.xmlHttp.setRequestHeader(key, this.options.requestHeaders[key]);
                }
            }
        }
 ...
 * @prop {number} [media.duration] Rendition of the media. Will override the info gotten from the player.
 * @prop {number} [media.bitrate] Bitrate of the media. Will override the info gotten from the player.
 * @prop {number|string} [media.rendition] Duration of the media. Either number "10000" or name "HighDef". Will override the info gotten from the player.
...
$YB.ResourceParser = function(context) {
    try {
        ...
        this.lastCDNRequestHeaders = '';
        this.lastCDNRequestResponse = null;
         ...
         * @readonly
         */
        this.cdnHost = undefined;

        /** Code of the Node Host type after parsing it.
         * @var {number}
         * @readonly
         */
        this.cdnType = undefined;
...
$YB.ResourceParser.prototype._init = function() {
    try {
        ...
        this.parseTimeout = setTimeout(function() {
            if ('ResourceParser' in that.context.comm._preloaders) {
 ...
 * CDNs that share requestHeaders will share request if they are one near the other, saving resources.
 */
...
$YB.ResourceParser.prototype.getNextNode = function() {
    try {
        if (this.context.data.parseCDNNodeHost) {
            ...
            this.cdns = $YB.ResourceParser.cdnsEnabled;
            if (this.cdns.length > 0 && !this.cdnHost) {
                ...
                if (typeof $YB.ResourceParser.cdnsAvailable[cdn] != 'undefined') {
                    ...
                    if (this.lastCDNRequestHeaders === JSON.stringify(config.requestHeaders)) {

                        this.parseCDN(config.headers, this.lastCDNRequestResponse);

                    } else {
                        this.lastCDNRequestHeaders = JSON.stringify(config.requestHeaders);
                        ...
                        new $YB.AjaxRequest(this.realResource, '', '', {
                            ...
                            that.lastCDNRequestResponse = httpEvent.target;
                            that.parseCDN(config.headers, httpEvent.target);
...
$YB.ResourceParser.prototype.parseCDN = function(headerParsers, response) {
    try {
        for (var key in headerParsers) {
            var match = null;
            try {
                match = headerParsers[key].regex.exec(response.getResponseHeader(headerParsers[key].name));
            } catch (e) {
                this.getNextNode();
                return;
            }
            ...
            if (match !== null) {
                switch (headerParsers[key].type) {
                    case 'host':
                        this.cdnHost = match[1];
                        break;
                    case 'type':
                        this.cdnType = this.parseCDNType(match[1]);
                        break;
                    case 'host+type':
                    default:
                        this.cdnHost = match[1];
                        this.cdnType = this.parseCDNType(match[2]);
                        break;
                }
            } else {
                this.getNextNode();
...
$YB.ResourceParser.prototype.parseCDNType = function(type) {
    try {
        switch (type) {
            case 'TCP_HIT':
            case 'HIT':
...
$YB.Api.prototype._init = function() {
    try {
        var report = "YAPI Modules Loaded: [Communication] [Adnalizer] ";
...
$YB.Api.prototype.getBitrate = function() {
    try {
        if (typeof this.data.media.bitrate != "undefined") {
            return this.data.media.bitrate;
        } else if (typeof this.plugin.getBitrate == "function" && this.plugin.getBitrate() != -1) {
