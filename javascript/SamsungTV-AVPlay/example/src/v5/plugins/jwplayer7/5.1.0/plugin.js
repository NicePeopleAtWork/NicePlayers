...
$YB.plugins.Jwplayer7 = function(player, options) {
    try {
        ...
        this.pluginVersion = "5.1.0-jwplayer7";
...
$YB.plugins.Jwplayer7.prototype.registerListeners = function() {
    try {
        ...
        $YB.utils.listenAllEvents(this.player.on, [
            'beforePlay', 'meta', 'playlistItem', 'setupError', 'idle', 'complete', 'buffer', 'bufferChange', 'firstFrame'
        ], function(e) {
        ...
        this.player.on('playlistItem', function(e) {
            try {
                ...
                // Set options
                plugin.data.setOptions(this.getPlaylistItem().youbora);
            } catch (error) {
                $YB.error(error);
            }
        });
        ...
        this.player.on('firstFrame', function(e) {
            try {
                $YB.debug(plugin.videoApi.chrono.joinTime.stop() + " vs " + e.loadTime);
                plugin.joinHandler();
            } catch (error) {
                $YB.error(error);
            }
