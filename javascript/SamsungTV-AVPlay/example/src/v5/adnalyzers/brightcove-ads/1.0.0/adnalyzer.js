/**
 * @license
 * Youbora 1.0.0-brightcove-ads Adnalyzer
...
if (typeof $YB != 'undefined') {
    ...
    $YB.adnalyzers.BrightcoveAds = function(plugin) {
        try {
            this.adnalyzerVersion = '1.0.0-brightcove-ads';

            // Reference to the plugin where it was called.
            if (plugin.player.ima3) {
                this.init(plugin, plugin.player.ima3);
            } else if (plugin.player.FreeWheelPlugin) {
                this.init(plugin, plugin.player.FreeWheelPlugin);
            } else {
