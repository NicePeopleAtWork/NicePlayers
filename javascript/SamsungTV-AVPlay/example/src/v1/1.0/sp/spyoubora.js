 ...
 * Author: Ferran Guti
 * Version: 1.0
 * 			- Detect Devices and Tech (Support HTML5, JWPlayer, Ooyala, PlayStation 3 and PhillipsTV, LG WebApps)
 * Version: 1.1
...
var spYoubora = 
{
	...
	youboraServer: "http://192.168.2.49/",
	urlLibs: "/js/libs/",
	urlBaseJS: "/js/", 		
	init: function() 
	{
		console.log("YOUBORA HERE!");
		if(!spYoubora.isSPYouboraLodad) {
			spYoubora.isSPYouboraLodad = true;
  			...
  			console.log("URL 1 : " +  spYoubora.urlLibs);
  			console.log("URL 2 : " +  spYoubora.urlBaseJS);
  			if(this.debug)
  			{
    ...
    collectParams: function () {
        try {
            ...
            for (var i = 0; i < scripts.length; i++) {
                if (scripts[i].src.indexOf('spyoubora') != -1) {
                    index = i;
                }
            }
            ...
            for (var i = 0; i < Pairs.length; i++) {
                ...
                if (this.debug) {
                    console.log("spYoubora :: collectParams :: " + key + " = " + val);
                }
            }
        } catch (error) {
            if (this.debug) {
                console.log("spYoubora :: collectParams :: Error :: " + error);
            }
        }
    },
	...
	checkDevice: function() 
	{
		...
		else if ( spYoubora.userAgent.search("googletv")		>-1 ) { spYoubora.foundDevice = spYoubora.deviceNames.LGTV; } 
		else if ( spYoubora.userAgent.search("LG")				>-1 ) { spYoubora.foundDevice = spYoubora.deviceNames.LGTV; }
		else if ( spYoubora.userAgent.search("LGE")				>-1 ) { spYoubora.foundDevice = spYoubora.deviceNames.LGTV; }
		else if ( spYoubora.userAgent.search("lg")				>-1 ) { spYoubora.foundDevice = spYoubora.deviceNames.LGTV; }
		else if ( spYoubora.userAgent.search("lge")				>-1 ) { spYoubora.foundDevice = spYoubora.deviceNames.LGTV; }
	...
	{ 
		try
		{
			if ( spYoubora.foundDevice == spYoubora.deviceNames.PC ) 
			{ 
	...
	addLibraries: function()
	{
		...
		{ 
			...
			if ( spYoubora.isSmartPluginLoaded () == false )
				 spYoubora.loadJavascriptFile ( spYoubora.urlBaseJS + 'viera-html5/sp.js' , function () { SmartPlugin.Init();} );
			else
				 SmartPlugin.Init(); 
		} 
...
try {
    ...
    try {
        ...
        if (spYoubora.debug) {
            console.log("spYoubora :: Init :: Error: " + error);
        }
    }
}