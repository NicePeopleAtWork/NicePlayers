...
YouboraAnalytics = function(playerId, mb, namespace) {
    ...
    // configuration
    this.pluginVersion = "2.2.3_desktop_newooyala";
...
YouboraAnalytics.prototype = {
    ...
    ***********************************************/
    ...
    onContentTreeFetchedHandler: function(event, contentTree) {
        ...
        try {
            if (this.isFlashPlugin) {
                if (typeof contentTree.isLiveStream != "undefined") {
        ...
        try {
            if (typeof contentTree.title != "undefined" && this.errorLastEvent != null) {
                // If we have an error pending, we send the error just now with the title
                this.forceSendError();
            } else if (this.resource && typeof contentTree.title != "undefined" && this.startLastTime > 0) {
                // If we have an error pending, we send the error just now with the title
                this.sendStartDelay();
            }
        } catch (err) {
        ...
        try {
            if (typeof embedCode != "undefined") {
                if (typeof embedCode == "object") {
                    ...
                    this.resource = embedCode.streamUrl;
                    this.youboraData.properties.content_metadata.title = embedCode.title;
                } else {
            ...
            var code = embedCode.embedCode;
            if (typeof code == "undefined") {
                code = embedCode;
            }


            if (this.lastEmbedCode != code) {
                this.lastEmbedCode = code;
                if (this.isStartEventSent) {
                    this.stop();
                } else {
                    this.reset();
