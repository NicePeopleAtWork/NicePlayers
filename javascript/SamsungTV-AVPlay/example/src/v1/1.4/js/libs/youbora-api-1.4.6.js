...
function YouboraCommunication(system, service, bandwidth, pluginVersion, targetDevice, playerId) {
    try {
                    ...
                    this.xmlHttp.onreadystatechange = function() {
                        if (this.readyState == 4) {
                            if (this.status == 200) {
                                ...
                                var newUrl = resource;
                                for (var key in file) {
                                    var matches = /.*((\.m3u8|\.ts|\.mp4))/i.exec(file[key]);
        ...
        if (typeof this.getYouboraData() != "undefined") {
            ...
            if (this.debug) {
                console.log("YouboraCommunication :: Unable to reach this.getYouboraData() :: Concurrency / Resume / Level3 :: Disabled");
            }
        }
...
YouboraCommunication.prototype.init = function() {
    try {
        ...
        var urlDataWithCode = this.service + "/data?system=" + this.system + "&pluginVersion=" + this.pluginVersion + "&targetDevice=" + this.targetDevice + "&outputformat=" + this.outputFormat + "&timemark=" + new Date().getTime();
...
YouboraCommunication.prototype.resourceTreatment = function() {
    try {
        ...
        if (this.debug) {
            console.log("YouboraCommunication :: resourceTratment " + err);
        }
    }
};
    ...
    if (typeof this.getYouboraData() != "undefined") {
        ...
        if (this.getYouboraData().getMediaResource().length > 0) {
            try {
                ...
                if (this.debug) {
                    console.log("YouboraCommunication :: HTTP LEVEL3 Header Request :: " + this.getYouboraData().getMediaResource());
                }
            } catch (error) {
                if (this.debug) {
...
YouboraCommunication.prototype.parseL3Header = function(header, typeCall) {
    try {
        ...
        if (this.debug) {
            console.log("YouboraCommunication :: Level3 :: onLoad :: Host: " + this.nodeHostDataStart.host + " :: Type: " + this.nodeHostDataStart.type);
        }
        this.getYouboraData().setCDNNodeDataObtained(true);
...
YouboraCommunication.prototype.parseHighwindsHeader = function(header, typeCall) {
    try {
        ...
        if (response.type == "p") {
            ...
            if (this.debug) {
                console.log("YouboraCommunication :: Highwinds :: Unknown type received: " + response.type);
            }
        ...
        if (this.debug) {
            console.log("YouboraCommunication :: Highwinds :: onLoad :: Host: " + this.nodeHostDataStart.host + " :: Type: " + this.nodeHostDataStart.type);
        }
        this.getYouboraData().setCDNNodeDataObtained(true);
        ...
        this.getYouboraData().setCDNNodeData(false);
        if (this.debug) {
            console.log("YouboraCommunication :: Highwinds :: Error with header, disabling header check" + error);
        }
        ...
        if (this.getYouboraData().getMediaResource().length > 0) {
            ...
            this.xmlHttp.addEventListener("load", function(httpEvent) {
                ...
                try {
                    ...
                    if (this.debug) {
                        console.log("YouboraCommunication :: Highwinds :: Error parsing header" + e);
                    }
                }
...
YouboraCommunication.prototype.parseAkamaiHeader = function(header) {
    try {
        ...
        if (this.debug) {
            console.log("YouboraCommunication :: Akamai :: onLoad :: Host: " + this.nodeHostDataStart.host + " :: Type: " + this.nodeHostDataStart.type);
        }
...
YouboraCommunication.prototype.enableResume = function() {
    try {
        ...
        clearInterval(this.resumeInterval);
        if (this.debug) {
            console.log("YouboraCommunication :: enableResume :: Error: " + err);
        }
    }
};
...
YouboraCommunication.prototype.disableResume = function() {
    try {
        ...
        clearInterval(this.resumeInterval);
        if (this.debug) {
            console.log("YouboraCommunication :: disableResume :: Error: " + err);
        }
    }
};
...
YouboraCommunication.prototype.sendStart = function(totalBytes, referer, properties, isLive, resource, duration, transcode) {
    try {
        ...
        if (this.debug) {
            console.log("YouboraCommunication :: sendStart :: " + err);
        }
    }
...
YouboraCommunication.prototype.sendError = function(errorCode, message) {
    try {
        ...
        if (this.debug) {
            console.log("YouboraCommunication :: sendError :: " + err);
        }
    }
};
...
YouboraCommunication.prototype.sendAdvancedError = function(errorCode, player, message, totalBytes, referer, properties, isLive, resource, duration, transcode) {
    try {
        ...
        if (this.debug) {
            console.log("YouboraCommunication :: sendAdvancedError :: " + err);
        }
    }
};
...
YouboraCommunication.prototype.enableConcurrency = function() {
    try {
        ...
        clearInterval(this.resumeInterval);
        if (this.debug) {
            console.log("YouboraCommunication :: enableConcurrency :: Error: " + err);
        }
    }
};
...
YouboraCommunication.prototype.disableConcurrency = function() {
    try {
        ...
        clearInterval(this.resumeInterval);
        if (this.debug) {
            console.log("YouboraCommunication :: disableConcurrency :: Error: " + err);
        }
    }
};
...
YouboraCommunication.prototype.loadAnalytics = function(httpEvent) {
    ...
    try {
        ...
        if (this.debug) {
            console.log("YouboraCommunication :: loadAnalytics :: Error: " + err);
        }
    }
};
...
YouboraCommunication.prototype.cPing = function() {
    try {
        ...
        if (this.debug) {
            console.log("YouboraCommunication :: cPing :: " + err);
        }
    }
};
...
YouboraCommunication.prototype.informEvent = function(event) {
    try {
        ...
        if (this.debug) {
            console.log("YouboraCommunication :: HTTP Request :: " + url);
        }
        this.xmlHttp.open("GET", url, true);
        this.xmlHttp.send();
    } catch (err) {
        if (this.debug) {
            console.log("YouboraCommunication :: getYouboraData :: " + informEvent);
        }
    }
}
...
YouboraCommunication.prototype.getYouboraData = function() {
    try {
        ...
        if (this.debug) {
            console.log("YouboraCommunication :: getYouboraData :: " + err);
        }
    }
};
