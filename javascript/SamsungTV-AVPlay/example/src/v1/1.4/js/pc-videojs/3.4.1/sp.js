...
videojs.plugin('youbora', function(params) {
	...
	var pluginVersion = "1.4.3.4.1_videojs";
	//is bcove
	if(player.mediainfo) {
		targetDevice = "PC-VIDEOJS-BCOVE";
		pluginName = "BCOVE";
		pluginVersion = "1.4.3.4.1_bcovevjs ";
	}
	var initDone = 0;
	var balanceObject = "";
	var balanceIndex = 1;
				...
				case "timeupdate":
					try {
						if (!isAdvertisment) {
							...
							if (isStartSent && !isJoinSent && isCanplayFired /*isPlayingFired && isLoadedDataFired && isProgressFired || isStalledFired)*/ ) {
