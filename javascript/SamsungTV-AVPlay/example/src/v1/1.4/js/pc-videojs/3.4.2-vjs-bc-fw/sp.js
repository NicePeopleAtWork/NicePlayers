 ...
 * Author: Biel Conde
 * Version: 1.4.3.4.2-vjs-bc-fw_videojs
 */
...
videojs.plugin('youbora', function(params) {
	...
	var pluginVersion = "1.4.3.4.2-vjs-bc-fw_videojs";
	//is bcove
	if(player.mediainfo) {
		targetDevice = "PC-VIDEOJS-BCOVE";
		pluginName = "BCOVE";
		pluginVersion = "1.4.3.4.2-vjs-bc-fw_bcovevjs ";
	}
	var initDone = 0;
	var balanceObject = "";
	var balanceIndex = 1;
	...
	this.youbora = {
		...
		checkPlayState: function(e) {
			...
			switch (e.type) {
				case "timeupdate":
					try {
						if ( isStartSent && firstTimeUpdate === 0 && (isCanplayFired || isProgressFired)) {
							firstTimeUpdate = Math.abs(new Date().getTime() - joinTimeBegin); //Only the first timeupdate

						} else if (isStartSent &&  firstTimeUpdate != -1 && new Date().getTime() - joinTimeBegin > 10000 && (isCanplayFired || isProgressFired)) {
						...
						if (!isAdvertisment) {
							...
							if (isStartSent && (!isJoinSent /*|| isBuffering*/) && isAdEndFired && !seeking /*isPlayingFired && isLoadedDataFired && isProgressFired || isStalledFired)*/ ) {
								player.youbora.setBufferEvent(mediaEvents.BUFFER_END);
							}
