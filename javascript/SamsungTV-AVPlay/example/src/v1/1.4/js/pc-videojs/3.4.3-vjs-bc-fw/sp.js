 ...
 * Author: Biel Conde
 * Version: 3.4.3-vjs-bc-fw_videojs
 * Comments: this plugin has disabled "abort" event since in Safari and Android causes double views. 
 */
...
videojs.plugin('youbora', function (params) {
	...
	var pluginVersion = "3.4.3-vjs-bc-fw_videojs";
	//is bcove
	if (player.mediainfo) {
		targetDevice = "PC-VIDEOJS-BCOVE";
		pluginName = "BCOVE";
		pluginVersion = "3.4.3-vjs-bc-fw_bcovevjs";
	}
	var initDone = 0;
	var balanceObject = "";
	var balanceIndex = 1;
	...
	this.youbora = {
		...
		bindEvents: function () {
			try {
				var playerEvents = ["canplay", "playing", "waiting", "timeupdate", "ended", "play", "pause", "error", "abort", "seeking", "seeked", "stalled", "suspend", "progress", "dispose", "loadeddata", "loadstart", "adstart", "adend", "select", "videoChange"];
				...
				player.playlist.on("select", function (e) {
					player.youbora.checkPlayState(e);
				});

				player.playlist.on("videoChange", function (e) {
					player.youbora.checkPlayState(e);
				});

				window.on("videoChange", function (e) {
					player.youbora.checkPlayState(e);
				});
		...
		isAndroidOrCanPlayFired: function () {
		...
		checkPlayState: function (e) {
			...
			case "timeupdate":
				try {
					if (isStartSent && firstTimeUpdate === 0 && isCanplayFired) {
						if (joinTimeBegin == 0) {
							joinTimeBegin = startTime;
						}
						firstTimeUpdate = Math.abs(new Date().getTime() - joinTimeBegin); //Only the first timeupdate

					} else if (isStartSent && firstTimeUpdate != -1 && (new Date().getTime() - joinTimeBegin > 10000) && isCanplayFired) {
						if (!isAdsFired && !isJoinSent) {
							isJoinSent = true;
							isBuffering = false;
							apiClass.sendJoin(currentTime, firstTimeUpdate);
							joinTimeBegin = 0;
						}
						firstTimeUpdate = -1;
					} else {
						if (!isAdvertisment) {
							if (isStartSent && !isJoinSent && isAdEndFired && !seeking && player.youbora.isAndroidOrCanPlayFired()) {
							...
							if (currentTime > 0) {
								...
								if (isJoinSent && (previousElapsedTime + bufferTimeThreshold) < videoPlayer.currentTime()) {
			...
			case "play":
				try {
					/*if (!isJoinSent && !isAdvertisment && !seeking) {
					...
					if (!isAdvertisment) {
						...
						if (!seeking) {
							...
							if (!isStartSent && videoPlayer.error() == null) {
								...
								//} while (previousResource == videoPlayer.currentSrc());

								previousResource = videoPlayer.currentSrc();
								...
								// to set options only first load pluging
								_options = null;
			...
			case "pause":
				try {
					if (!isAdvertisment && !seeking && isStartSent && isJoinSent) {
						apiClass.sendPause();
						isPaused = true;
					}
				} catch (error) {
		...
		checkBuffering: function () {
			try {
				if (isBuffering && isStartSent && isJoinSent && !isAdvertisment) {
					if (!isAdsFired) {
						player.youbora.setBufferEvent(mediaEvents.BUFFER_END);
					}
					bufferTryCount = 0;
		...
		setBufferEvent: function (bufferState) {
			try {
				...
				case mediaEvents.BUFFER_END:
					...
					if (bufferTimeBegin == 0) {
						bufferTimeBegin = startTime;
						startTime = 0;
					}
	...
	if (typeof YouboraCommunication != 'undefined' && _youboraData != 'undefined') {
		player.youbora.setProperties(_options);
		player.youbora.Init();
	}
