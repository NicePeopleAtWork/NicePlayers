 ...
 * Author: Biel Conde
 * Version: 1.4.3.4.3-vjs-bc-ima_videojs
 */
...
videojs.plugin('youbora', function(params) {
	...
	try {
		...
		var pluginVersion = "1.4.3.4.3-vjs-bc-ima_videojs";
		//is bcove
		if(player.mediainfo) {
			targetDevice = "PC-VIDEOJS-BCOVE";
			pluginName = "BCOVE";
			pluginVersion = "1.4.3.4.3-vjs-bc-ima_bcovevjs ";
		}
		var initDone = 0;
		var balanceObject = "";
		var balanceIndex = 1;
		...
		this.youbora = {
			...
			getYouboraData: function() {
				try {
					...
					if (debug) {
						console.log("SmartPlugin :: " + pluginName + " :: getYouboraData :: Error: " + error);
					}
				}
			},
			...
			setBcoveProperties: function(options) {
				...
				try {
					if(player.mediainfo) {
						...
						if(options.properties.content_metadata.title == undefined) {
							options.properties.content_metadata.title = player.mediainfo.name;
						}

						if(options.properties.content_metadata.duration == undefined) {
							options.properties.content_metadata.duration = player.mediainfo.duration;
							duration = player.mediainfo.duration;
						} else {
							duration = options.properties.content_metadata.duration || duration;
						}				
					}				
				} catch (e) {
				} finally {
			...
			isAd: function(adPlaying) {
				try {
					...
					if (debug) {
						console.log("SmartPlugin :: " + pluginName + " :: isAd :: Error: " + error);
					}
				}
			},
			...
			checkPlayState: function(e) {
				try {
					...
					//console.log(e.type);
					//$('#console').append('<h1>'+ e.type +'</h1>');

					switch (e.type) {
						case "timeupdate":
							try {
								if (!isAdvertisment) {
									if (isStartSent && !isJoinSent && isAdsFinished && player.youbora.mobileOrDesktopJointime()) {
										//send jointime
									...
									if (currentTime > 0) { 
										// for detect canplay event only when ads finished
										isAdsFinished = true;

										//if previous+e < currentTime -->Playback re-started
										if ((previousElapsedTime + bufferTimeThreshold) < videoPlayer.currentTime() && isJoinSent) {
						...
						case "error":
							try {
								...
								if ((isNaN(duration)) || (duration == "NaN") || duration == NaN || duration == Infinity || duration < 0) {
									duration = 0;
								}
								...
								if (_youboraData.getBalanceEnabled() && apiClass.enableBalancer) {
									...
									isStreamError = true;
									apiClass.sendAdvancedError("131" + getBalancerErrorCount(), playerId, "CDN_PLAY_FAILURE", 0, window.location.href, _options, _youboraData.getLive(), balanceObject[balanceIndex]['URL'], duration);
									balanceIndex++;
									...
									apiClass.sendAdvancedError(error, playerId,message, 0, window.location.href, _options, _youboraData.getLive(), youboraData.getMediaResource(), duration);
									player.youbora.reset();
								}
							} catch (error) {
								if (debug) {
							...
							try {
								...
								if (!isStartSent && !isJoinSent && player.playlist().length > 0) {
									...
									if (!isStartSent && videoPlayer.error() == null) {
										...
										player.youbora.setPing();
										//player.youbora.setBufferChecker(); // moved to jointime
										isStartSent = true;
									}
								}						
							} catch (error) {
					...
					if (debug) {
						console.log("SmartPlugin :: " + pluginName + " :: checkPlayState :: Error :: End of mirrors error:" + error);
					}
				}
			},
			...
			setBufferChecker: function() {
				try {
					...
					if (debug) {
						console.log("SmartPlugin :: " + pluginName + " :: setBufferChecker :: Error :: End of mirrors error:" + error);
					}
				}
			},
			...
			getBalancerErrorCount: function() {
				try {
					...
					if (debug) {
						console.log("SmartPlugin :: " + pluginName + " :: getBalancerErrorCount :: Error :: End of mirrors error:" + error);
					}
				}
			},
			...
			mobileOrDesktopJointime: function() {
				try {
					if(typeof window.orientation !== 'undefined'){
						return videoPlayer.currentTime() > 0;
					} else if(player.youbora.ieVersion() > 0) {
						return (isAdsFinished && player.youbora.ieVersion() > 0);
					}else {
						return (isCanplayFired || isProgressFired);
					}				
				} catch (err) {
					if (debug) {
						console.log("SmartPlugin :: " + pluginName + " :: mobileOrDesktopJointime :: Error: " + err);
					}
				}
			},
		...
		if (debug) {
			console.log("SmartPlugin :: " + pluginName + " :: Contructor :: Error: " + error);
		}
	}
});