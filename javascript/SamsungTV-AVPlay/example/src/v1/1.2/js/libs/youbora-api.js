            ...
            if (resumeUserid.length > 0) {
                try {
                    if (youboraData.getResumeEnabled()) {
                        ...
                        if (this.debug) { console.log("YouboraCommunication :: checkResumeState :: Resume is now disabled in data."); }
                    }
                } catch (error) {
                    clearInterval(this.resumeInterval);
                    if (this.debug) {
        ...
        if (httpEvent.target.readyState == 4) {
            ...
            if (response > 0) {
                ...
                if (typeof resumeCallback == "function") {
                   ...
                   try { console.log(eval(resumeCallback)); } catch (e) { console.log(e)};
                } else {
                    if (this.debug) {
                ...
                clearInterval(this.resumeInterval);
                if (this.debug) {
                    console.log("YouboraCommunication :: Resume :: Empty response... stoping resume.");
                }
            }
        }
...
YouboraCommunication.prototype.getAkamaiHeader = function () {
    try{
        ...
        this.xmlHttp.open("head", youboraData.getMediaResource(), true);
                    //this.xmlHttp.setRequestHeader('X-Cache', '');
        this.xmlHttp.send();
    }catch(err){
        youboraData.setCDNNodeData(false);
...
YouboraCommunication.prototype.sendErrorWithParameters = function (errorCode, message, totalBytes, referer, properties, isLive, resource, duration, transcode) {
    try { 
       if (typeof errorCode == "undefined"  || parseInt(errorCode) < 0) {
        ...
        if (transcode == undefined || transcode == "undefined") { transcode = youboraData.getTransaction(); }
        if (duration == undefined || duration == "undefined") { duration = 0; }
...
YouboraCommunication.prototype.init = function () {
    try {
        ...
        // TODO Add timeout to Data request!!
    ...
    if (typeof youboraData != "undefined") {
        ...
        try {
            ...
            if (youboraData.getConcurrencyEnabled()) {
                ...
                if (mainContext.debug) { console.log("YouboraCommunication :: checkConcurrencyWork :: HTTP Concurrency Request :: " + urlDataWithCode); }
            } else {
                if (mainContext.debug) { console.log("YouboraCommunication :: checkConcurrencyWork :: Concurrency disabled in data."); }
            }

        } catch (err) {
            if (mainContext.debug) { console.log("YouboraCommunication :: startConcurrencyWork :: Error: " + err); }
        }
    } else {
        if (this.debug) { console.log("YouboraCommunication :: Concurrency :: Unable to reach youboraData"); }
    }
};
