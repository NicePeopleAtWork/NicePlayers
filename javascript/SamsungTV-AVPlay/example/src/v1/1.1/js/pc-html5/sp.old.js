...
function SmartPluginAnalytics ( playerObj , system, service, bandwidth )
{
    ...
    // configuration
    this.pluginVersion      = "2.0.0._html5";
    this.targetDevice       = "HTML5_VideoPlayer";
...
SmartPluginAnalytics.prototype.setBalancedResource = function(obj)
{
    console.log(obj);
};
...
// TODO: plugin with GOTOPOS (Resume)
...
var SmartPlugin = { 
    ...
    Init: function()  {  
        ...
        else 
        {
            try
            {
                ...
                try {
                    if(youboraData.getMediaResource().length > 0 && youboraData.getBalanceEnabled())
                    {
                        var path = youboraData.getMediaResource().replace(/^.*\/\/[^\/]+/, '')
                        SmartPlugin.smartAnalytics.communication.getBalancedResource(path, function(obj) { SmartPlugin.setBalancedResource(obj); });
                    }
                } 
                catch (e)
                {
                    console.log('Error while getting balanced resource: '+ e)
                }
    ...
    setBalancedResource: function(obj) { 
        if(obj != false)
        {
            ...
            { 
                document.getElementsByTagName("video")[0].src = obj['1']['URL']; 
            }
            else 
            {
        ...
        {
            console.log('Invalid response from balancer')
        }
    }
};
