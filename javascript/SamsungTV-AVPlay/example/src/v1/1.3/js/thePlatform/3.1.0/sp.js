...
$pdk.plugin.Smartplugin = $pdk.extend(function () {}, {
    ...
    constructor: function () {
        ...
        this.pluginVersion = "1.3.3.1.0_platHTML5",
        ...
        this.balancerObject = null;
        this.originalCDN = youboraData.getCDN();
        this.balancerIndex = 1;
        this.isBalancing = false;
        this.isResumingBalance = false;
        this.previousTime;
        this.resourceToLoad = '';
    ...
    initialize: function (lo) {
        ...
        $pdk.controller.addEventListener("OnLoadReleaseUrl", this.onLoadReleaseUrl);
        $pdk.controller.addEventListener("OnResetPlayer", this.onResetPlayer);
    ...
    onMediaError: function (e) {
        ...
        if (this.isBalancing) {
            ...
            $pdk.controller.resetRelease();
            this.communications.sendErrorWithParameters("1310" + (this.balancerIndex - 1), "", 0, window.location.href, youboraData, this.isLive, this.urlResource, this.duration);
            this.balancerCallback(this.balancerObject);
        } else {
    ...
    onMediaStart: function (e) {
        ...
        if (this.originalResource == "") {
            this.originalResource = e.data.URL;
        }
        if (youboraData.getBalanceEnabled() && this.communications.enableBalancer) {
            if (this.balancerObject == null) {
                var path = this.communications.getResourcePath(this.originalResource);
                this.communications.getBalancedResource(path, this.balancerCallback);
            }
        }
    ...
    onResetPlayer: function() {
        $pdk.controller.loadReleaseURL(this.resourceToLoad, true);
    },
    ...
    balancerCallback: function (balancerObject) {
        youboraData.log(balancerObject);
        //Plugin is the plugin object, instantiated below
        plugin.balancerObject = balancerObject;
        if (this.balancerObject != false) {
            var newUrl = "";
            if (balancerObject[plugin.balancerIndex] != undefined) {
                newUrl = balancerObject[plugin.balancerIndex].URL;
                youboraData.setCDN(balancerObject[plugin.balancerIndex].CDN_CODE);
                youboraData.log("Balancing: Trying Url : " + newUrl);
                plugin.isBalancing = true;
            } else {
                //newUrl = "http://mirrorblender.top-ix.org/movies/sintel-1024-surround.mp4";
                newUrl = plugin.originalResource;
                youboraData.setCDN(plugin.originalCDN);
                youboraData.log("Balancing: Original Url : " + newUrl);
                plugin.isBalancing = false;
            }
            console.log('Smartswitch: '+ newUrl);
            var smil = '<smil xmlns="http://www.w3.org/2005/SMIL21/Language"><body><ref src="' + newUrl + '"/></body></smil>'
            
            if(this.currentTime == 0) {
                $pdk.controller.setSmil(smil);
            } else {
                this.previousTime = this.currentTime;
                this.isResumingBalance = true;
                this.resourceToLoad = newUrl;
                $pdk.controller.setSmil(smil);
                $pdk.controller.loadReleaseURL(newUrl, true);
                //$pdk.controller.seekToPosition(previousTime);
                //$pdk.controller.clearCurrentRelease();
                //$pdk.controller.endCurrentRelease();
                //$pdk.controller.clickPlayButton();
            }
            
            plugin.balancerIndex++;
        }
    }
