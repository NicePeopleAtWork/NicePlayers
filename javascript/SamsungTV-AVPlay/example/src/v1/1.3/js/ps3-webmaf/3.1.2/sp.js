...
var SmartPlugin = {
    ...
    secondsCounter: {
        start: function() { SmartPlugin.internalTimer = setInterval(function() { SmartPlugin.currentTime++; } ,1000) },
        pause: function() { clearInterval(SmartPlugin.internalTimer); },
        reset: function() { clearInterval(SmartPlugin.internalTimer); SmartPlugin.currentTime = 0; } 
    },
    ...
    setAccessFunction: function () {        
        ...
        try {
            window.accessfunction = function (json) {
                if (SmartPlugin.debug) { console.log("SmartPlugin :: " + SmartPlugin.pluginName + " :: accessfunction :: " + json); }

                try { var data = JSON.parse(json); } catch (e) { 
                ...
                switch (data.command) {
                    case 'playerError':
                        try {
                            ...
                            if (SmartPlugin.balancing && SmartPlugin.communicationClass.enableBalancer) {
                                ...
                                SmartPlugin.communicationClass.sendErrorWithParameters("3001", "PLAY_FAILURE", 0, window.location.href, "", youboraData.getLive(), SmartPlugin.urlResource, SmartPlugin.duration); 
                                SmartPlugin.reset();
                            } 
                        } catch (error) {
                            if (SmartPlugin.debug) {
                    ...
                    case 'getPlaybackTime':
                        ...
                        if (SmartPlugin.previousElapsedTime != data.elapsedTime) { SmartPlugin.checkBuffering(); }
    ...
    replaceFunctions: function () {
        try {
            ...
            window.external.user = function (json) {
                if (SmartPlugin.debug) {  console.log("SmartPlugin :: " + SmartPlugin.pluginName + " :: external :: " + json); }
                var data = JSON.parse(json); 
                switch (data.command) {
                    case "load":  
                        try { 
                            if (typeof data.fileName != "undefined") { SmartPlugin.urlResource = data.fileName; } 
                            if (typeof data.contentUri != "undefined") { SmartPlugin.urlResource = data.contentUri; } 
                            if (typeof data.licenseUri != "undefined") { SmartPlugin.DRMLicense = data.licenseUri; }
                            if (typeof data.customData != "undefined") { SmartPlugin.DRMCustomData = data.customData; }
                        ...
                        } catch (error) { console.log("SmartPlugin :: " + SmartPlugin.pluginName + " :: window.external.user :: Error: " + error); }
                    break;
                }
                window.externalUserFunction(json);
            }
        } catch ( error ) {
            if (SmartPlugin.debug) { console.log("SmartPlugin :: " + SmartPlugin.pluginName + " :: replaceFunctions :: Error: " + error); }
        }
    },
    checkBuffering: function () {
        if (SmartPlugin.buffering) { if (SmartPlugin.isStartSent) { SmartPlugin.setBufferEvent(SmartPlugin.SmartPluginsEvents.BUFFER_END); } }
                ...
                case SmartPlugin.SmartPluginsEvents.BUFFER_END:
                    ...
                    if (SmartPlugin.isJoinEventSent == false) {
                        ...
                        if (SmartPlugin.isStartSent) {
                            ...
                            if (SmartPlugin.debug) { console.log("SmartPlugin :: " + SmartPlugin.pluginName + " :: setBufferEvent :: Join sent!"); }
                        }
                    } else {
    ...
    ping: function () {
        try { 
            if (SmartPlugin.isStartSent && !SmartPlugin.paused) {
                var videoBtr = "-1"
                try { videoBtr = videometrics.currentBitrate; } catch (e) { } 
                if (videoBtr == 0) { videoBtr = "-1"; }
    ...
    statusChange: function (status) {
        try {
            ...
            if (SmartPlugin.debug) { console.log("SmartPlugin :: " + SmartPlugin.pluginName + " :: statusChange :: "+ status); }
            switch (status) {
                case 'paused':
                    if (SmartPlugin.isStartSent) {
                ...
                case 'stopped':
                    if (SmartPlugin.isStartSent) { SmartPlugin.reset(); }
                break;
                ...
                case 'playing':
                    ...
                    if (SmartPlugin.paused) { 
                        SmartPlugin.communicationClass.sendResume(); SmartPlugin.paused = false; 
                    } 
                    if (!SmartPlugin.isJoinEventSent) { SmartPlugin.setBufferEvent(SmartPlugin.SmartPluginsEvents.BUFFER_END); }
                    SmartPlugin.checkBuffering();
                break;
                ...
                case 'opening':
                    if(!SmartPlugin.isJoinEventSent) { SmartPlugin.setBufferEvent(SmartPlugin.SmartPluginsEvents.BUFFER_BEGIN); }
    ...
    reset: function () {
        try {
            clearInterval(SmartPlugin.headTimer);
            clearInterval(SmartPlugin.pingTimer);
            SmartPlugin.secondsCounter.reset();
