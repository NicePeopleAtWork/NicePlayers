 ...
 var SmartPlugin = { 
    ...
    unbindEvents: function ()
    {
        try
        {
            ...
            for ( elem in playerEvents ) 
            { 
                SmartPlugin.videoPlayer.off(playerEvents[elem],function ( e ) {});
            }   
        }
        catch ( error )
        {
    ...
    isAd: function( adPlaying ) {
        if ( SmartPlugin.debug || true){
            console.log ( "SmartPlugin :: "+ SmartPlugin.pluginName  +" :: isAd :: "+ adPlaying);
        }
        SmartPlugin.isAdvertisment = adPlaying;
    }
    ,
                ...
                {  
                    if(!SmartPlugin.isAdvertisment) {
                        ...
                        if ( !SmartPlugin.isStartSent )
                        {
                            ...
                            var resource = SmartPlugin.urlResource;
                            if(youboraData.getMediaResource()!=""){
                                resource =youboraData.getMediaResource();
                            }
                            SmartPlugin.apiClass.sendStart ( 0 , window.location.href , "" , youboraData.getLive() , resource , SmartPlugin.duration ,youboraData.getTransaction());
            ...
            case "error":
                try 
                {
                    if ( youboraData.getBalanceEnabled() && SmartPlugin.apiClass.enableBalancer )
                    {
                        ...
                        SmartPlugin.apiClass.sendErrorWithParameters("131" + SmartPlugin.getBalancerErrorCount(), "CDN_PLAY_FAILURE",SmartPlugin.targetDevice, 0, window.location.href, "", youboraData.getLive(), SmartPlugin.balanceObject[SmartPlugin.balanceIndex]['URL'], SmartPlugin.videoPlayer.duration());
                    ...
                    else
                    {
                        ...
                        var resource=SmartPlugin.videoPlayer.currentSrc();
                        if(youboraData.getMediaResource()!=""){
                            resource =youboraData.getMediaResource();
                        }
                        SmartPlugin.apiClass.sendErrorWithParameters(error, SmartPlugin.targetDevice,message, 0, window.location.href, "", youboraData.getLive(), resource,  SmartPlugin.videoPlayer.duration());
            ...
            case "playing": 
                try
                {
                    if(!SmartPlugin.isAdvertisment) {
                        ...
                        if ( !SmartPlugin.isJoinSent ) {     SmartPlugin.setBufferEvent(SmartPlugin.mediaEvents.BUFFER_END); }
         ...
         if ( SmartPlugin.videoPlayer.currentTime() > 0 && !SmartPlugin.isBuffering && !SmartPlugin.isPaused && !SmartPlugin.seeking) {
                    var now = new Date().getTime();
                      if ( (SmartPlugin.previousElapsedTime >= SmartPlugin.videoPlayer.currentTime()) && (SmartPlugin.videoPlayer.currentTime() < SmartPlugin.videoPlayer.duration())) {
                        ...
                        console.log("SmartPlugin.bufferTryCount  : " + SmartPlugin.bufferTryCount);
    ...
    {    
        try 
        {
            ...
            else
            {
                ...
                if ( SmartPlugin.debug )
                {
                    console.log("SmartPlugin :: "+ SmartPlugin.pluginName  +" :: Balancer :: End of mirrors"); 
                }   
            }
        } 
        catch ( error )
