...
var SmartPlugin = { 
    ...
    checkPlayState: function ( e ) 
    {
        ...
        switch ( e.type )
        {
            ...
            case "error":
                try 
                {
                    if ( youboraData.getBalanceEnabled() && SmartPlugin.apiClass.enableBalancer )
                    {
                        ...
                        SmartPlugin.apiClass.sendErrorWithParameters("131" + SmartPlugin.getBalancerErrorCount(), "CDN_PLAY_FAILURE", 0, window.location.href, "", youboraData.getLive(), SmartPlugin.urlResource, SmartPlugin.duration);
                    ...
                    else
                    {
                        ...
                        SmartPlugin.apiClass.sendErrorWithParameters(error, "PLAY_FAILURE", 0, window.location.href, "", youboraData.getLive(), SmartPlugin.urlResource, SmartPlugin.duration);
            ...
            */
            case "progress":
                /*console.log("PROGRESS : Previous Time : " + SmartPlugin.previousElapsedTime + " , current : "+ SmartPlugin.videoPlayer.currentTime +" , isBuffering : " + SmartPlugin.isBuffering +" , previous+threshold : "+ (SmartPlugin.previousElapsedTime + SmartPlugin.bufferTimeThreshold )+" , diff : "+ (SmartPlugin.previousElapsedTime - SmartPlugin.videoPlayer.currentTime ));
                if ( SmartPlugin.currentTime > 0 & !SmartPlugin.isBuffering && !SmartPlugin.isPaused && !SmartPlugin.seeking) {
                    var now = new Date().getTime();
                    if((SmartPlugin.endSeekDate + SmartPlugin.endSeekThreshold) > now){
    ...
    {
        bufferCheckTimer = setTimeout(function(){ SmartPlugin.bufferChecker(); }, 250);
                ...
                //    console.log("IN the first condition");
                    var now = new Date().getTime();
                      ...
                      //  if (SmartPlugin.bufferTryCount >=2 ) {
                                 //   console.log("STARTBUFFER  : Previous Time : " + SmartPlugin.previousElapsedTime + " ,  SmartPlugin.videoPlayer.currentTime  : "+  SmartPlugin.videoPlayer.currentTime  +" , SmartPlugin.previousElapsedTime >= SmartPlugin.videoPlayer.currentTime : "+ ( SmartPlugin.previousElapsedTime >= SmartPlugin.videoPlayer.currentTime) +", SmartPlugin.bufferTryCount : " +SmartPlugin.bufferTryCount);
            ...
            { 
                ...
                { 
                    ...
                    if(obj['1']['URL'] != SmartPlugin.urlResource)
                    {
                        ...
                        try
                        {
                            //console.log(SmartPlugin.videoPlayer.src);
                        ...
                        catch ( error )
                        {
                            ...
                            SmartPlugin.videoPlayer.setAttribute("src",SmartPlugin.urlResource);
                            SmartPlugin.videoPlayer.load();
                            SmartPlugin.videoPlayer.play();
                        } 
                    } 
    ...
    isDevicePC : function(){
        ...
        try{
            ...
            if (navigator.userAgent.search("iphone") > -1)              { isDevicePc =false } 
            else if (navigator.userAgent.search("ipad") > -1)           { isDevicePc =false } 
            else if (navigator.userAgent.search("android 2") > -1)      { isDevicePc =false } 
            else if (navigator.userAgent.search("android 4") > -1)      { isDevicePc =false } 
            else if (navigator.userAgent.search("playstation 3") > -1)  { isDevicePc =false } 
            else if (navigator.userAgent.search("playstation 4") > -1)  { isDevicePc =false } 
            else if (navigator.userAgent.search("appletv") > -1)        { isDevicePc =false }
            else if (navigator.userAgent.search("lgtv") > -1)           { isDevicePc =false }
            else if (navigator.userAgent.search("netcast") > -1)        { isDevicePc =false } 
            else if (navigator.userAgent.search("lg") > -1)             { isDevicePc =false }
            else if (navigator.userAgent.search("smarttv") > -1)        { isDevicePc =false } 
            else if (navigator.userAgent.search("philipstv") > -1)      { isDevicePc =false } 
            else if (navigator.userAgent.search("philips") > -1)        { isDevicePc =false } 
            else if (navigator.userAgent.search("roku") > -1)           { isDevicePc =false } 

            else if (navigator.userAgent.search("x86") > -1)            { isDevicePc =true } 
            else if (navigator.userAgent.search("x64") > -1)            { isDevicePc =true }  
            else if (navigator.userAgent.search("wow64") > -1)          { isDevicePc =true } 
        }catch(e){
            console.log(e);
        }
        return isDevicePc;
