...
var SmartPlugin = {
	...
	pluginVersion: "1.3.3.2.0_bc_HTML5",
	initDone: 0,
	// Balancer
	...
	startPlugin: function () {
		...
		try {
			...
			for (var key = 0; key < iframes.length; key++) {
				if (iframes[key].src.indexOf("brightcove") > -1) {
					try {
						test = brightcove.api.getExperience(iframes[key].id);
					} catch (error) {
						if (SmartPlugin.debug) {
							console.log("SmartPlugin :: " + SmartPlugin.pluginName + " :: brightcove.api.getExperience :: Error: " + error);
						}
					}
				}
			}
			if (SmartPlugin.initDone <= 5) {
				if (test == undefined) {
					if (SmartPlugin.debug) {
						console.log("SmartPlugin :: " + SmartPlugin.pluginName + " :: startPlugin :: BrightCove experience not ready retryng...");
					}
					setTimeout("SmartPlugin.startPlugin()", 500);
				} else {
					try {
						SmartPlugin.player = test;
						...
						if (SmartPlugin.debug) {
							console.log("SmartPlugin :: " + SmartPlugin.pluginName + " :: Player Not Found: " + error);
						}
					}
				}
			} else {
				if (SmartPlugin.debug) {
					console.log("SmartPlugin :: " + SmartPlugin.pluginName + " :: startPlugin Fatal Error :: Unable to reach BrightCove Experience...");
				}
			}
		} catch (error) {
			if (SmartPlugin.debug) {
	...
	checkPositions: function (time) {
		SmartPlugin.previousElapsedTime = SmartPlugin.currentTime;
		SmartPlugin.currentTime = time;
		if (!SmartPlugin.isJoinSent && SmartPlugin.currentTime > 0) {
			SmartPlugin.setBufferEvent(SmartPlugin.mediaEvents.BUFFER_END);
		} else {
			if (SmartPlugin.previousElapsedTime > 0) {
				if (SmartPlugin.currentTime == SmartPlugin.previousElapsedTime) {
					if (SmartPlugin.isStartSent) {
						if (!SmartPlugin.isPaused && !SmartPlugin.isSeeking) {
	...
	checkTimer: function () {
		SmartPlugin.videoPlayer.getVideoPosition(SmartPlugin.checkPositions);
	...
	bindEvents: function () {
		try {
			...
			try {
				SmartPlugin.videoPlayer.addEventListener(brightcove.api.events.MediaEvent.CHANGE, SmartPlugin.checkPlayState);
			} catch (error) {
				if (SmartPlugin.debug) {
					console.log("SmartPlugin :: " + SmartPlugin.pluginName + " :: bindEvents :: Failed to bind [MediaEvent.CHANGE] :: " + error);
				}
			}

			try {
				SmartPlugin.videoPlayer.addEventListener(brightcove.api.events.MediaEvent.ERROR, SmartPlugin.checkPlayState);
			} catch (error) {
				if (SmartPlugin.debug) {
					console.log("SmartPlugin :: " + SmartPlugin.pluginName + " :: bindEvents :: Failed to bind [MediaEvent.ERROR] :: " + error);
				}
			}

			try {
				SmartPlugin.videoPlayer.addEventListener(brightcove.api.events.MediaEvent.PLAY, SmartPlugin.checkPlayState);
			} catch (error) {
				if (SmartPlugin.debug) {
					console.log("SmartPlugin :: " + SmartPlugin.pluginName + " :: bindEvents :: Failed to bind [MediaEvent.PLAY] :: " + error);
				}
			}

			try {
				SmartPlugin.videoPlayer.addEventListener(brightcove.api.events.MediaEvent.PROGRESS, SmartPlugin.checkPlayState);
			} catch (error) {
				if (SmartPlugin.debug) {
					console.log("SmartPlugin :: " + SmartPlugin.pluginName + " :: bindEvents :: Failed to bind [MediaEvent.PROGRESS] :: " + error);
				}
			}

			try {
				SmartPlugin.videoPlayer.addEventListener(brightcove.api.events.MediaEvent.STOP, SmartPlugin.checkPlayState);
			} catch (error) {
				if (SmartPlugin.debug) {
					console.log("SmartPlugin :: " + SmartPlugin.pluginName + " :: bindEvents :: Failed to bind [MediaEvent.STOP] :: " + error);
				}
			}

			try {
				SmartPlugin.videoPlayer.addEventListener(brightcove.api.events.MediaEvent.BEGIN, SmartPlugin.checkPlayState);
			} catch (error) {
				if (SmartPlugin.debug) {
					console.log("SmartPlugin :: " + SmartPlugin.pluginName + " :: bindEvents :: Failed to bind [MediaEvent.BEGIN] :: " + error);
				}
			}

			try {
				SmartPlugin.videoPlayer.addEventListener(brightcove.api.events.MediaEvent.COMPLETE, SmartPlugin.checkPlayState);
			} catch (error) {
				if (SmartPlugin.debug) {
					console.log("SmartPlugin :: " + SmartPlugin.pluginName + " :: bindEvents :: Failed to bind [MediaEvent.COMPLETE] :: " + error);
				}
			}

			try {
				SmartPlugin.videoPlayer.addEventListener(brightcove.api.events.MediaEvent.BUFFER_COMPLETE, SmartPlugin.checkPlayState);
			} catch (error) {
				if (SmartPlugin.debug) {
					console.log("SmartPlugin :: " + SmartPlugin.pluginName + " :: bindEvents :: Failed to bind [MediaEvent.BUFFER_COMPLETE] :: " + error);
				}
			}

			try {
				SmartPlugin.videoPlayer.addEventListener(brightcove.api.events.MediaEvent.BUFFER_BEGIN, SmartPlugin.checkPlayState);
			} catch (error) {
				if (SmartPlugin.debug) {
					console.log("SmartPlugin :: " + SmartPlugin.pluginName + " :: bindEvents :: Failed to bind [MediaEvent.BUFFER_BEGIN] :: " + error);
				}
			}

			try {
				SmartPlugin.videoPlayer.addEventListener(brightcove.api.events.MediaEvent.SEEK, SmartPlugin.checkPlayState);
			} catch (error) {
				if (SmartPlugin.debug) {
					console.log("SmartPlugin :: " + SmartPlugin.pluginName + " :: bindEvents :: Failed to bind [MediaEvent.SEEK] :: " + error);
				}
			}

			try {
				SmartPlugin.videoPlayer.addEventListener(brightcove.api.events.MediaEvent.SEEK_NOTIFY, SmartPlugin.checkPlayState);
			} catch (error) {
				if (SmartPlugin.debug) {
					console.log("SmartPlugin :: " + SmartPlugin.pluginName + " :: bindEvents :: Failed to bind [MediaEvent.SEEK] :: " + error);
				}
	...
	sendStartHandler: function (video) {
		try {
			...
			if (youboraData.properties.content_metadata.title == "") {
				if (SmartPlugin.debug) {
					console.log("SmartPlugin :: " + SmartPlugin.pluginName + " :: Setting Title: " + video.displayName);
				}
				youboraData.setPropertyMetaTitle(video.displayName);
			}
			if (youboraData.getLive() == false) {
			...
			if (SmartPlugin.urlResource != "") {
				if (SmartPlugin.balancing && SmartPlugin.apiClass.enableBalancer) {
					if (SmartPlugin.balanceObject == "") {
						...
						SmartPlugin.apiClass.sendErrorWithParameters("130" + SmartPlugin.getBalancerErrorCount(), "CDN_PLAY_FAILURE_AND_TRY_NEXT", 0, window.location.href, youboraData, youboraData.getLive(), SmartPlugin.urlResource, SmartPlugin.duration, youboraData.getTransaction());
			...
			if (SmartPlugin.debug) {
				console.log("SmartPlugin :: " + SmartPlugin.pluginName + " :: sendStartHandler :: Error: " + error);
			}
		}
	...
	checkPlayState: function (e) {
		...
		case "mediaProgress":
			try {
				...
				if (SmartPlugin.debug) {
					console.log("SmartPlugin :: " + SmartPlugin.pluginName + " :: mediaProgress :: Error: " + error);
				}
			}
			break;
		...
		case "mediaPlay":
			try {
				...
				if (SmartPlugin.debug) {
					console.log("SmartPlugin :: " + SmartPlugin.pluginName + " :: mediaPlay :: Error: " + error);
				}
			}
			break;
		...
		case "mediaStop":
			try {
				if (e.position >= SmartPlugin.duration && SmartPlugin.isStartSent && SmartPlugin.duration > 0) {
					//SmartPlugin.checkBuffering();
				...
				if (SmartPlugin.debug) {
					console.log("SmartPlugin :: " + SmartPlugin.pluginName + " :: mediaStop :: Error: " + error);
				}
			}
			break;
		case "templateError":
		case "mediaError":
			try {
				if (SmartPlugin.balancing && SmartPlugin.apiClass.enableBalancer) {
					...
					var niceErrorCode;
					try {
						niceErrorCode = SmartPlugin.errorCodes[e.code];
						if (niceErrorCode == undefined) {
							niceErrorCode = e.code;
						}
					} catch (e) {}
					if (niceErrorCode == undefined) {
						niceErrorCode = "3001";
					}
					SmartPlugin.apiClass.sendErrorWithParameters(niceErrorCode.toString(), e.errorType, 0, window.location.href, youboraData, youboraData.getLive(), SmartPlugin.urlResource, SmartPlugin.duration, youboraData.getTransaction());
					SmartPlugin.reset();
				}
		...
		case "mediaBufferComplete":
			try {
				...
				if (SmartPlugin.debug) {
					console.log("SmartPlugin :: " + SmartPlugin.pluginName + " :: mediaBufferComplete :: Error: " + error);
				}
			}
		...
		case "mediaBufferBegin":
			try {
				...
				if (SmartPlugin.debug) {
					console.log("SmartPlugin :: " + SmartPlugin.pluginName + " :: mediaBufferBegin :: Error: " + error);
				}
			}
			break;
		...
		case "mediaBegin":
			try {
				...
				//Timing is measured taking into account the timeupdate frequency
				SmartPlugin.progressInterval = setInterval(function () {
					SmartPlugin.checkTimer()
				}, 400);
			} catch (error) {
				if (SmartPlugin.debug) {
	...
	setBrightCoveURL: function (url) {
		...
		if (SmartPlugin.debug) {
			console.log("SmartPlugin :: BRIGHTCOVE :: setBrightCoveURL :: " + url);
		}
		try {
			...
			try {
				SmartPlugin.contentPlayer.updateMedia(SmartPlugin.bcvideoplayer);
			} catch (e) {}
			try {
				SmartPlugin.videoPlayer.loadVideoByID(SmartPlugin.bcvideoplayer.id);
			} catch (e) {}
			if (!SmartPlugin.isStartSent) {
				SmartPlugin.isStartSent = true;
	...
	refreshBalancedResource: function () {
		try {
			if (typeof SmartPlugin.balanceObject[SmartPlugin.balanceIndex]['URL'] != "undefined") {
				...
				SmartPlugin.setBrightCoveURL(SmartPlugin.balanceObject[SmartPlugin.balanceIndex]['URL']);
			} else {
				SmartPlugin.balancing = false;
				if (SmartPlugin.debug) {
					console.log("SmartPlugin :: " + SmartPlugin.pluginName + " :: Balancer :: Error :: End of mirrors");
				}
				SmartPlugin.apiClass.sendErrorWithParameters("131" + SmartPlugin.getBalancerErrorCount(), "CDN_PLAY_FAILURE", 0, window.location.href, youboraData, youboraData.getLive(), SmartPlugin.urlResource, SmartPlugin.duration, youboraData.getTransaction());
				...
				if (typeof obj['1']['URL'] != "undefined") {
					...
					if (obj['1']['URL'] != SmartPlugin.urlResource) {
						...
						try {
							...
							SmartPlugin.setBrightCoveURL(SmartPlugin.videoPlayer.currentSrc);
						} catch (error) {
							if (SmartPlugin.debug) {
	...
	setBufferEvent: function (bufferState) {
		try {
			...
			case SmartPlugin.mediaEvents.BUFFER_BEGIN:
				clearInterval(SmartPlugin.progressInterval);
	...
	ping: function () {
		try {
			...
			SmartPlugin.setPing();
			if (SmartPlugin.isStartSent) {
				SmartPlugin.apiClass.sendPingTotalBitrate(SmartPlugin.currentBitrate, SmartPlugin.currentTime);
			}
		} catch (error) {
			if (SmartPlugin.debug) {
	...
	reset: function () {
		try {
			clearTimeout(SmartPlugin.pingTimer);
			clearTimeout(SmartPlugin.progressInterval);
