...
//Custom fixes for wildhitz.
//The overlappedPlayList parameter should
//be parametrized and set by default to 0
...
var template = function(player, config, div){
    ...
    function beforePlayHandler(event)
    {
        ...
        //it starts playing.
        if (!isStartEventSent && (!isBalancerActive() ||  overlappedPlayList == true) && numStarts==0){
        ...
        switch (type)
        {
            ...
            case "start":
                try{
                    ...
                    if(mediaAttributes!=null && duration <=0){
            ...
            case "error":
                ...
                if(isStartEventSent){
                    try{
                        communications.sendErrorWithParameters(SmartPluginError[errorObj.message],parseError(errorObj),transaction,resourceToPass, pamSystem,isLive,pamMediaProperties,pamUser,windowLocation,"0",pingTimeInSeconds,pluginVersion);
                    }catch(e){
                        youboraData.log(e);
                    }
