...
package
{
	...
	{   
		...
		protected var pluginTimeBegin:uint;
		protected var joinTimeBegin:uint = 0;
		...
		protected var pluginVersion:String = "3.1.6_BrightCove_SWF";
		...
		{ 
			...
			{  
				...
				if (!isJoinEventSent)
				{
					...
					if(joinTimeBegin==0){
						joinTimeBegin = pluginTimeBegin;
					}
					if(joinTimeBegin==0){
						joinTimeBegin = joinTimeEnd - 1000;	
					}					
		...
		public function sendAnalytics(type:String, actPamCode:String, time:Number=0, duration:Number=0, errorCode:String="", bitrate:Number=0, resourceURLSended:String="", diffTimeLastPing:Number=0):void
		{
			...
			switch (type)
			{
				...
				case "start":  
					...
					if ( extraparam1 != ""  ) { string += "&param1="  + extraparam1; }
					if ( extraparam2 != ""  ) { string += "&param2="  + extraparam2; }
					if ( extraparam3 != ""  ) { string += "&param3="  + extraparam3; }
					if ( extraparam4 != ""  ) { string += "&param4="  + extraparam4; }
					if ( extraparam5 != ""  ) { string += "&param5="  + extraparam5; }
					if ( extraparam6 != ""  ) { string += "&param6="  + extraparam6; }
					if ( extraparam7 != ""  ) { string += "&param7="  + extraparam7; }
					if ( extraparam8 != ""  ) { string += "&param8="  + extraparam8; }
					if ( extraparam9 != ""  ) { string += "&param9="  + extraparam9; }
					if ( extraparam10 != "" ) { string += "&param10=" + extraparam10; }
				...
				case "error":
					...
					if (balanceEnabled == false) {
						...
						request = new URLRequest(pamErrorUrl+"?system="+escape(accountCode)+"&user="+escape(username)+"&pluginVersion="+escape(pluginVersion)+
							...
							"&properties="+properties+"&errorCode="+escape(errorCode)+"&code="+escape(actPamCode)+"&diffTime="+diffTimeLastPing+string);
