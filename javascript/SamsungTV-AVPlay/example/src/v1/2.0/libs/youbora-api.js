...
YouboraCommunication.prototype.getLevel3Header = function () {
    if (typeof youboraData != "undefined" && this.fastDataValid) {
        ...
        if (youboraData.getMediaResource().length > 0) {
            ...
            youboraData.setCDNNodeData(false);
            if (this.debug) {
                youboraData.log("YouboraCommunication :: Level3 :: No mediaResource specified, disabling first header check");
            }
        }
    }
}
...
YouboraCommunication.prototype.enableResume = function () {
    try {
        ...
        clearInterval(this.resumeInterval);
        youboraData.log("YouboraCommunication :: enableResume :: Error: " + error);
    }
};
...
YouboraCommunication.prototype.disableResume = function () {
    try {
        ...
        clearInterval(this.resumeInterval);
        youboraData.log("YouboraCommunication :: disableResume :: Error: " + error);
    }
};
...
YouboraCommunication.prototype.parseL3Header = function (header, obj) {
    try {
        ...
        if (obj == 1) {
            ...
            youboraData.log("YouboraCommunication :: Level3 :: onLoad :: Host: " + this.l3dataStart.host + " :: Type: " + this.l3dataStart.type);
        } else {
            this.l3dataPing.host = l3Response.host;
            this.l3dataPing.type = l3Response.type;
            youboraData.log("YouboraCommunication :: Level3 :: beforeStart :: Host: " + this.l3dataPing.host + " :: Type: " + this.l3dataPing.type);
        }
        return true;
    } catch (error) {
        youboraData.setCDNNodeData(false);
...
YouboraCommunication.prototype.parseAkamaiHeader = function (header) {
    try {
        ...
        youboraData.log("YouboraCommunication :: Akamai :: onLoad :: Host: " + this.l3dataStart.host + " :: Type: " + this.l3dataStart.type);
        return true;
    } catch (error) {
        youboraData.setCDNNodeData(false);
...
YouboraCommunication.prototype.sendStartL3 = function (totalBytes, referer, properties, isLive, resource, duration, transcode) {
    try {
        ...
        youboraData.log("YouboraCommunication :: sendStartL3 :: Error: " + error);
    }
}
            ...
            this.xmlHttp.addEventListener("load", function (httpEvent) {
                try {
                    ...
                    if (this.context.parseL3Header(header, 2)) {
                        ...
                        youboraData.log("YouboraCommunication :: Level3 :: Error parsing header");
                        this.context.sendStartL3(totalBytes, referer, properties, isLive, resource, duration, transcode);
                    }
                } catch (error) {
                    youboraData.setCDNNodeData(false);
...
YouboraCommunication.prototype.enableConcurrency = function () {
    try {
        ...
        clearInterval(this.resumeInterval);
        youboraData.log("YouboraCommunication :: enableConcurrency :: Error: " + error);
    }
};
...
YouboraCommunication.prototype.disableConcurrency = function () {
    try {
        ...
        clearInterval(this.resumeInterval);
        youboraData.log("YouboraCommunication :: disableConcurrency :: Error: " + error);
    }
};
...
YouboraCommunication.prototype.getBalancedResource = function (path, callback, context) {
    if (!youboraData.enableBalancer) {
        ...
        try {
            ...
            this.xmlHttp.addEventListener("load", function (httpEvent) {
                ...
                if (error == false) {
                    ...
                    mainContext.balancedCallback(objJSON,context);
                } else {
                    mainContext.balancedCallback(false,context)
                }
            }, false);
            this.xmlHttp.open("GET", urlDataWithCode, true);
            this.xmlHttp.send();
