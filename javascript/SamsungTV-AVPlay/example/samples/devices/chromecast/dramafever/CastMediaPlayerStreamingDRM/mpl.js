...
var title = null //title of the current video
  ...
  **/
  ...
  mediaManager.onLoad = function(event) {
    ...
    if (event.data['media'] && event.data['media']['contentId']) {
      ...
      title = event.data.customData.payload.title;
      window.updateProperties(title);
      ...
      if (url.lastIndexOf('.m3u8') >= 0) {
        ...
        url.lastIndexOf('.isml/') >= 0 || url.lastIndexOf('Manifest') >= 0) {
      ...
      if (protocol == null) {
        ...
        console.log('### Play with HTML5 player');
        SmartPlugin.disableGoogleMediaPlayer();
      } else {
        ...
        SmartPlugin.setGoogleMediaPlayer(mediaPlayer);
        if (liveStreaming) {
