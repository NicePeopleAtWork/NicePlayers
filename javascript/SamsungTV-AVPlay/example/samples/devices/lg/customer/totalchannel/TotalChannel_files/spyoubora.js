...
var spYoubora = {
    ...
    init: function () {
        try {
            ...
            if (!spYoubora.isSPYouboraLodad) {
                ...
                if (this.debug) { console.log('spYoubora :: Init'); }
                ...
                if (spYoubora.debug) { console.log('spYoubora :: Current User-Agent: ' + spYoubora.userAgent); }
                if (spYoubora.debug) { console.log('spYoubora :: Checking Device...'); }
                spYoubora.checkDevice();
                if (spYoubora.debug) { console.log('spYoubora :: Checking Technology...'); }
                spYoubora.checkTechnology();
                if (spYoubora.debug) { console.log('spYoubora :: Adding libraries...'); }
                spYoubora.addLibraries();
            } else {
                if (spYoubora.debug) { console.log("spYoubora :: Already Initialized... "); }
            }
        } catch (error) {
            if (spYoubora.debug) { console.log("spYoubora :: init :: Error: " + error); }
        }
    },
    ...
    technologyNames: { UNKNOWN: 0, HTML5: 1, JWPLAYER: 2, OOYALA: 3, BCOVER: 4, SILVERLIGHT_NODRM: 5, SILVERLIGHT_DRM: 6, VIDEOJS: 7, CHROMECAST: 8 },
    collectParams: function () {
        try {
            ...
            for (var i = 0; i < Pairs.length; i++) {
                ...
                if (this.debug) { console.log("spYoubora :: collectParams :: " + key + " = " + val); }
            }
        } catch (error) {
            if (this.debug) { console.log("spYoubora :: collectParams :: Error :: " + error); }
        }
    },
    checkDevice: function () {
        try {
            ...
            if (spYoubora.debug) { console.log("spYoubora :: checkDevice :: Error: " + error); }
        }
    },
    checkTechnology: function () {
        try {
            if (spYoubora.foundDevice == spYoubora.deviceNames.PC || spYoubora.foundDevice == spYoubora.deviceNames.MAC) {
                ...
                var video = null;
                try { video = document.getElementsByTagName("video")[0]; } catch (error) { if (spYoubora.debug) { console.log("spYoubora :: No <video> found: " + error); } }
                ...
                else if (chrome.cast != null) { spYoubora.foundTechnology = spYoubora.technologyNames.CHROMECAST; }
                else if (video != null) { spYoubora.foundTechnology = spYoubora.technologyNames.HTML5; }
            }
        } catch (error) {
            if (spYoubora.debug) { console.log("spYoubora :: checkTechnology :: Error: " + error); }
        }
    },
    addLibraries: function () {
        try {
            ...
            if (((spYoubora.foundDevice == spYoubora.deviceNames.PC) || (spYoubora.foundDevice == spYoubora.deviceNames.MAC)) && spYoubora.foundTechnology == spYoubora.technologyNames.HTML5) {
                ...
                /*if (spYoubora.isSmartPluginLoaded() == false) {
                    if (this.forcedVersion != undefined) {
                        ...
                        spYoubora.loadJavascriptFile(spYoubora.urlBaseJS + 'all-jwplayer/sp.js', function () { SmartPlugin.Init(); });
                    }
                } else { SmartPlugin.Init(); }*/
            ...
            } else if (spYoubora.foundTechnology == spYoubora.technologyNames.CHROMECAST) {
                if (spYoubora.isYouboraApiLoaded() == false) { spYoubora.loadJavascriptFile(spYoubora.urlLibs + 'youbora-api.js', function () {}); }
                if (spYoubora.isSmartPluginLoaded() == false) {
                    if (this.forcedVersion != undefined) {
                        spYoubora.loadJavascriptFile(spYoubora.urlBaseJS + 'chromecast/' + this.forcedVersion + '/sp.js', function () { SmartPlugin.Init(); });
                    } else {
                        spYoubora.loadJavascriptFile(spYoubora.urlBaseJS + 'chromecast/sp.js', function () { SmartPlugin.Init(); });
                    }
                } else { SmartPlugin.Init(); }
                ...
                if (spYoubora.isSmartPluginLoaded() == false) {
                    if (this.forcedVersion != undefined) {
                        ...
                        spYoubora.loadJavascriptFile(spYoubora.urlBaseJS + 'pc-videojs/sp.js', function () { SmartPlugin.Init(); });
                    }
                } else { SmartPlugin.Init(); }
                ...
                if (spYoubora.isSmartPluginLoaded() == false) {
                    if (this.forcedVersion != undefined) {
                        ...
                        spYoubora.loadJavascriptFile(spYoubora.urlBaseJS + 'lg/sp.js', function () { SmartPlugin.Init(); });
                    }
                } else { SmartPlugin.Init(); }
                ...
                if (spYoubora.isSmartPluginLoaded() == false) {
                    if (this.forcedVersion != undefined) {
                        ...
                        spYoubora.loadJavascriptFile(spYoubora.urlBaseJS + 'silverlight/no-drm/sp.js', function () { SmartPlugin.Init(); });
                    }
                } else { SmartPlugin.Init(); }
                ...
                if (spYoubora.isSmartPluginLoaded() == false) {
                    if (this.forcedVersion != undefined) {
                        ...
                        spYoubora.loadJavascriptFile(spYoubora.urlBaseJS + 'silverlight/drm/sp.js', function () { SmartPlugin.Init(); });
                    }
                } else { SmartPlugin.Init(); }
            } else {
                if (spYoubora.debug) { console.log("spYoubora :: No devices found..."); }
            }
        } catch (error) {
            if (spYoubora.debug) { console.log("spYoubora :: addLibraries :: Error: " + error); }
        }
    },
    isYouboraDataLoaded: function () {
        try {
            ...
            if (spYoubora.debug) { console.log("spYoubora :: YouboraData is loaded :: " + isLoaded); }
            return isLoaded;
        } catch (error) {
            if (spYoubora.debug) { console.log("spYoubora :: isYouboraDataLoaded :: Error: " + error); }
            return false;
        }
    },
    isYouboraApiLoaded: function () {
        try {
            ...
            if (spYoubora.debug) { console.log("spYoubora :: YouboraApi is loaded :: " + isLoaded); }
            return isLoaded;
        } catch (error) {
            if (spYoubora.debug) { console.log("spYoubora :: isYouboraApiLoaded :: Error: " + error); }
            return false;
        }
    },
    isSmartPluginLoaded: function () {
        try {
            ...
            if (spYoubora.debug) { console.log("spYoubora :: SmartPlugin is loaded :: " + isLoaded); }
            return isLoaded;
        } catch (error) {
            if (spYoubora.debug) { console.log("spYoubora :: isSmartPluginLoaded :: Error: " + error); }
            return false;
        }
    },
    loadJavascriptFile: function (url, callback) {
        ...
        } catch (error) { if (spYoubora.debug) { console.log("spYoubora :: loadJavascriptFile :: Error :: " + error); } }
    },
    reload: function () {
        ...
        spYoubora.youboraVersion = "1.2";
        spYoubora.youboraServer = "http://smartplugin.youbora.com/";
        spYoubora.youboraPreServer = "http://smartplugin.youbora.com/pre/";
