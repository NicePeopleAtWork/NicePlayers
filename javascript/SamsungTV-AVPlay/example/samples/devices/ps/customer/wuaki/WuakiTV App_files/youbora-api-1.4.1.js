            ...
            if(extension == 'm3u8') {
                ...
                this.xmlHttp.onreadystatechange = function() {
                    if (this.readyState == 4 && this.status == 200) {
                        ...
                        for(var key in file) {
                            try {
                                var extension = /\.([0-9a-zA-Z]+)(?:[\?#]|$)/.exec( file[key] )[1];
                                extension = extension.toLowerCase();
                            } catch (e) {}
                            if(extension == 'm3u8' || extension == 'mp4' || extension == 'ts'){
                                newUrl = file[key];
                                break;
                            }
                        }
                        if(file[key].indexOf('http://') > -1 || file[key].indexOf('https://') > -1) {
                            ...
                            that.getRealResourceHLS( absoluteURL + newUrl, callback);
                        }
    ...
    try {
        if (this.canSendEvents && this.fastDataValid) {
            ...
            if( url.indexOf( this.pamStartUrl ) != -1) {
                urlDataWithCode += "&resource=" + encodeURIComponent( youboraData.getMediaResource() );
            }
