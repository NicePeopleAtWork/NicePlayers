...
videojs.plugin('youbora', function(params) {
	...
	this.youbora = {
		...
		bindEvents: function() {
			try {
				var playerEvents = ["canplay", "playing", "waiting", "timeupdate", "ended", "play", "pause", "error", "abort", "seeking", "seeked", "stalled", "suspend", "progress", "dispose", "loadeddata"];
		...
		checkPlayState: function(e) {
			...
			console.log('# '+ e.type);						// to delete
			$('#console').append('<p>'+ e.type +' - '+ player.duration() +'</p>');		// to delete
			switch (e.type) {
				case "timeupdate":
					try {
						if (!isAdvertisment) {
							...
							// workaround playlists, because not send play/playing events in firstplay
							if(player.playlist().length > 0 && !playlistsFirstPlay) {
								playlistsFirstPlay = true;
								if (!isJoinSent) {
				...
				case "play":
					try {
						//reset stop for playlists, known bug player  // to delete
						/*if(isStartSent && !isPaused && seeking) {
							player.youbora.reset();
						}*/
						
						//seeked
						if (!isAdvertisment && !isPaused && seeking) {
							player.youbora.setBufferEvent(mediaEvents.BUFFER_END);
							seeking = false;
							isBuffering = false;
